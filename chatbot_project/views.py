...
def chatBot(request):
    ...
    history = model.fit(
        np.array(train_x), np.array(train_y), epochs=20, batch_size=1, verbose=7
    )
    ...
    # Save bot_response to the Response table
    response_instance = Response.objects.create(
        text=json.dumps({"message": message, "bot_response": bot_response})
    )
